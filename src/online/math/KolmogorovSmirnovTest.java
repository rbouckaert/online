...
//    }
     ...
     * @since 3.4
     */
    final static double PI_SQUARED = Math.PI * Math.PI;
    ...
    private void checkArray(double[] array) {
        ...
        if (array.length < 2) {
            throw new IllegalArgumentException("InsufficientDataException(LocalizedFormats.INSUFFICIENT_OBSERVED_POINTS_IN_SAMPLE, array.length,2");
        }
    }
    ...
    public double exactP(double d, int n, int m, boolean strict) {
       return 1 - n(m, n, m, n, calculateIntegralD(d, m, n, strict), strict) /
               online.math.CombinatoricsUtils.binomialCoefficientDouble(n + m, m);
    }
    ...
    private static void fixTies(double[] x, double[] y) {
       ...
       final Random dist = new Random(100);
       ...
       boolean ties = true;
       do {
           jitter(x, dist, minDelta);
           jitter(y, dist, minDelta);
           ties = hasTies(x, y);
           ct++;
       } while (ties && ct < 1000);
       if (ties) {
     ...
     */
    private static void jitter(double[] data, Random dist, double minDelta) {
        for (int i = 0; i < data.length; i++) {
            data[i] += dist.nextDouble() * 2 * minDelta - minDelta;
        }
    }
